...
export default function MapDashboard() {
  ...
  const getCameraObstacles = (cameraId: number): ObstacleType[] => {
    const cameraObstacles = obstacles.filter(obstacle => obstacle.cameraId === cameraId);
              ...
              {loading ? (
                ...
                >
                  ...
                  {filteredCameras.map((camera) => {
                    ...
                    return (
                      <div key={camera.id} className={`circle-camera ${hasObstacleData ? 'has-obstacles' : ''}`}>
                        <Marker
                          position={[camera.latitude, camera.longitude]}
                          icon={hasObstacleData ? cameraWithObstacleIcon : cameraIcon}
                          eventHandlers={{ click: () => handleCameraSelect(camera) }}
                        >
                          <Popup maxWidth={420} maxHeight={500} className="custom-popup">
                              ...
                              <div className="relative px-4 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white flex-shrink-0">
                                <div className="flex items-center justify-between">
                                  <div className="flex-1 min-w-0 pr-4">
                                    <h3 className="font-semibold text-lg leading-tight truncate">{camera.name}</h3>
                                  </div>
                                </div>
                              </div>
                                ...
                                <div className="px-4 py-3">
                                  ...
                                  <div className="mb-3 p-3 bg-gray-50 rounded-lg border border-gray-100">
                                    ...
                                    <div className="text-sm text-gray-600 ml-6">
                                      <div className="mb-1 leading-relaxed">{camera.location}</div>
                                    ...
                                    {hasObstacleData ? (
                                      <div>
                                        <div className="flex items-center gap-2 mb-3 sticky top-0 bg-white z-10 py-1">
                                          ...
                                          <span className="font-semibold text-red-700 text-sm">Obstacles Detected</span>
                                        ...
                                        <div className="max-h-60 overflow-y-auto obstacles-scroll space-y-2 pr-1">
                                          {cameraObstacles.map((obstacle, index) => (
                                            <div key={obstacle.id} className="bg-gradient-to-r from-red-50 to-red-50/50 border border-red-200 rounded-lg p-3 hover:shadow-sm transition-shadow">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                  ...
                                                  <div className="ml-6">
                                                    <div className="text-xs text-red-600 mb-0.5 font-medium">
                                                      {getRelativeTime(obstacle.detectionTime)}
                                                    </div>
                                                    <div className="text-xs text-gray-500">
                                                      {formatDetectionTime(obstacle.detectionTime)}
                                                    </div>
                                                  </div>
                                                </div>
                                      ...
                                      <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-3">
                                        <div className="flex items-center gap-2">
                                          ...
                                          <span className="text-green-700 font-medium text-sm">All Clear</span>
                                        </div>
                                        <p className="text-green-600 text-xs mt-1 ml-4">No obstacles detected</p>
                                      </div>
                                    )}
                                  </div>
                                </div>
