...
def stream_overspeed_video_service(youtube_url: str, camera_id: int, speed_limit_kmh: float = 60.0, pixels_per_meter: float = 10.0):
    """
    Dịch vụ truyền luồng video để phát hiện và lưu vi phạm vượt tốc độ, tích hợp với API backend.
    ...
    # Lấy zones từ API, lọc các zone có type="line"
    zones = fetch_camera_config(camera_id)
    speed_lines_standard = [
        json.loads(zone["coordinates"])
        for zone in zones if zone.get("zoneType") == "line"
    ]
    if not speed_lines_standard:
        # Mặc định nếu không lấy được từ API
        speed_lines_standard = [
            [[0, 300], [640, 300]],  # Vạch 1
            [[0, 400], [640, 400]]   # Vạch 2
        ]
    ...
    def convert_coordinates_to_frame_size(standard_coords, frame_width, frame_height):
        ...
        return np.array([[int(round(x * scale_x)), int(round(y * scale_y))] for x, y in standard_coords], dtype=np.int32)
    ...
    try:
        while True:
            ...
            if not frame_size_initialized:
                ...
                print(f"Đã khởi tạo {len(speed_lines)} vạch đo tốc độ")

            # Vẽ vạch
            for i, line in enumerate(speed_lines):
                cv2.polylines(frame_annotated, [line], isClosed=False, color=(0, 255, 255), thickness=3)
                cv2.polylines(frame_for_video, [line], isClosed=False, color=(0, 255, 255), thickness=3)
